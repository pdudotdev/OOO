...
[![Stable Release](https://img.shields.io/badge/version-1.1.2-blue.svg)](https://github.com/pdudotdev/ooo/releases/tag/v1.1.2)
[![Last Commit](https://img.shields.io/github/last-commit/pdudotdev/ooo)](https://github.com/pdudotdev/ooo/commits/main)
...
* The framework is not an end all be all, not exhaustive, and can definitely be improved.
* It's relatively easy to implement in about 1-2 hours or so even if you're not a tech god.
* I am in no way affiliated with any of the tools, websites or services discussed below.
...
As an **OSINT investigator**, you are usually facing a plethora of challenges in the online space and you need to implement several concrete measures for:
* Maintaining your privacy and anonymity in regards to your target.
* Maintaining your browsing activities private in regards to online trackers.
* Maintaining a segregated space for your OSINT work (hardware, VMs, identity).
* Keeping your OSINT machine clean from malware and vulnerabilities.
* Keeping your OSINT tools and data securely stored and protected.
* Reducing your overall digital footprint and attack surface.
...
Now, the list that follows is by no means exhaustive, but it does contain most of the necessary precautions when it comes to securing and keeping the **BASE** safe. The purpose of each of the tasks below is to at least partially implement the threat model-related measures and principles outlined previously. Remember that **there's no such thing as perfect privacy and anonymity**, however we need to do our best to protect ourselves and our data.

## 🛠️ Task #1 

On your **BASE** machine, start fresh by installing Ubuntu 24.x. If you prefer other Linux distros, go ahead and install your favorite one, however this framework assumes Ubuntu as the OS. For the installation you'll need to download the [latest Ubuntu Desktop .iso image](https://ubuntu.com/download/desktop), then make a bootable USB drive with [Balena Etcher](https://etcher.balena.io/) or [Rufus](https://rufus.ie/en/).
...
* If you choose to go with the paid version of Proton VPN, then you can also enable the **Secure Core** option for extra security, which is `passing user traffic through multiple servers` as Proton describes it. It's basically a double-hop VPN setup activated with a single click. Read [this article](https://protonvpn.com/support/secure-core-vpn) to learn more about Secure Core, how it works and how to enable and configure it.
...
However:
* If you choose to stick with the Proton suite and use **ProtonPass**, keep in mind that it can be easily installed as a **browser extension** and used after logging in with your new Proton email and password. One thing to remember is once you installed the extension, make sure to go to its **Settings**, then in the **Security** tab check the **Extra password** option and set a password that will be required to use ProtonPass. This acts as an additional layer of protection for your sensitive data such as login information.
* On the other hand, if you prefer a 100% local solution, then go with **KeePassXC**. It's very important to remember to periodically (at least on a weekly basis) back-up the **KeePassXC database** - the *.kdbx* file - to an offline storage device which should preferably be encrypted as well. This way, no matter what happens to your **BASE** machine (gets compromised, corrupted or becomes unusable for some reason) you have your credentials and other sensitive data available at all times.
...
* If you're unsure about a website, paste the URL into [**URLScan.io**](https://urlscan.io/) to see any redirects, domain info and stats, and a screenshot of the homepage.
* **Do not plug in any random USB or storage devices**, especially if you've plugged them already into other machines. You risk bringing malware or other unwanted stuff into your **BASE** machine.
...
* In the **Additional Tasks** section above I stated: **Do not plug in any random USB or storage devices**. And again, you shouldn't.
* However, what you should do is to **periodically** back-up your OSINT work results and data to an encrypted offline storage device.
* I strongly advice you to acquire a **brand new (high-capacity) USB or SSD drive**, [encrypt it](https://www.youtube.com/watch?v=sWyBxfoc8fo) and back-up your data regularly.
* Needless to say, use this new USB or SSD drive **only** with your **BASE** machine and do not randomly plug it into any computer.
* In case something happens to your **BASE** machine, you need to always have a back-up of your work and the KeePassXC database file.
* An offline backup **protects your data and credentials** against the effects of malware, ransomware, data corruption or OS failure on your **BASE** machine.
...
* Using Tor or Tor-based operating systems such as Tails or Whonix will also introduce a degree of **latency** in your OSINT work, in addition to many websites blocking you right from the get-go.
